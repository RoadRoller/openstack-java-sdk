...
public class RoutersResource {
	...
	public UpdateRoute updateRoutes(String routerId, java.util.List<HostRoute> rs) {
		RouterForCreate router = new RouterForCreate();
		router.setRoutes(rs);
		return new UpdateRoute(routerId, router);
	}
...
//				return target.request(MediaType.APPLICATION_JSON).get(Networks.class);
	...
	public class UpdateRoute extends OpenStackRequest<Router> {
		public UpdateRoute(String routerId, RouterForCreate router) {
			super(CLIENT, HttpMethod.PUT, buildPath("routers/", routerId), Entity.json(router), Router.class);
		}
	}
